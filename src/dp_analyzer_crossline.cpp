...
CDPAnalyzerCrossLine<Dtype>::CDPAnalyzerCrossLine(const cv::Rect& roi, const cv::Point2i& p1, const cv::Point2i& p2) {
    roi_ = roi;
    p1_ = p1;
    p2_ = p2;
}
...
bool CDPAnalyzerCrossLine<Dtype>::Init() {
    ...
    cl_.Init(ep.model(), ep.prototxt(), pmap_(roi_), abs(roi_.tl().y - roi_.br().y), abs(roi_.br().x - roi_.tl().x));
    return true;
}
...
std::vector<Dtype> CDPAnalyzerCrossLine<Dtype>::Analyze(IN cv::Mat frame) {
    ...
    std::vector<float> predicted = cl_.CalcPredict(p1_, p2_, density_feature, x_feature, y_feature);
